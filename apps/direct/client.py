...
def start_job(url: str, prompt: str) -> dict:
    ...
    response = httpx.post(f"{url}/job", json={"prompt": prompt}, timeout=DEFAULT_TIMEOUT)
...
def get_job(url: str, job_id: str) -> str:
    ...
    response = httpx.get(f"{url}/job/{job_id}", timeout=DEFAULT_TIMEOUT)
    response.raise_for_status()
...
def list_jobs(url: str, archived: bool = False) -> str:
    ...
    response = httpx.get(f"{url}/jobs", params=params, timeout=DEFAULT_TIMEOUT)
    response.raise_for_status()
    return response.text
...
def clear_jobs(url: str) -> dict:
    ...
    response = httpx.post(f"{url}/jobs/clear", timeout=DEFAULT_TIMEOUT)
    response.raise_for_status()
    return response.json()
...
def latest_jobs(url: str, n: int = 1) -> str:
    ...
    response = httpx.get(f"{url}/jobs", timeout=DEFAULT_TIMEOUT)
    response.raise_for_status()
    ...
    for job in latest:
        job_id = job["id"]
        detail = httpx.get(f"{url}/job/{job_id}", timeout=DEFAULT_TIMEOUT)
...
def stop_job(url: str, job_id: str) -> dict:
    ...
    response = httpx.delete(f"{url}/job/{job_id}", timeout=DEFAULT_TIMEOUT)
    response.raise_for_status()
    return response.json()


def reset(url: str, mode: str = "soft") -> dict:
    """POST url/reset/{mode}, returns response dict."""
    response = httpx.post(f"{url}/reset/{mode}", timeout=30)
    response.raise_for_status()
    return response.json()
